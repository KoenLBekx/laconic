...
// TODO: Replace the § operator by k`. (This requires the ` operator to be
//				implemented differently, i.e. as an "alternative_count" property
//				in the expressions generated by fn make_tree, and marked as such in
//				Expression.get_representation().)
...
impl Expression {
    pub fn new(opr_mark: char) -> Self {
        let opr: OperatorFunc = match opr_mark {
            ...
            'k' => &opr_funcs::pop_stack,
            '§' => &opr_funcs::stack_depth,
...
mod tests {
    ...
    mod exec {
        ...
        fn x_stack_push_one_get_depth() {
            assert_eq!(1f64, Interpreter::execute("K5§".to_string()).unwrap().numeric_value);
        }
        ...
        fn x_stack_push_more_get_depth() {
            assert_eq!(4f64, Interpreter::execute("K(5 ~45 /41 2 #Aha!) §".to_string()).unwrap().numeric_value);
        }
